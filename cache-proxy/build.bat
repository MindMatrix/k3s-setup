...
REM docker build . -t mindmatrix/cache-proxy:%BUILD_NUMBER%
dotnet publish src\cache-proxy.csproj --os linux --arch x64 -c Release -p:ContainerImageTag=%BUILD_NUMBER%
if %errorlevel% neq 0 (
    echo Build failed. 
) else (
    docker push mindmatrix/cache-proxy:%BUILD_NUMBER%
    kubectl set image deployment/cache-proxy cache-proxy=mindmatrix/cache-proxy:%BUILD_NUMBER%
