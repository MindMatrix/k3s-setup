...
REM docker build . -t mindmatrix/forward-auth:%BUILD_NUMBER%
dotnet publish src\forward-auth.csproj --os linux --arch x64 -c Release -p:ContainerImageTag=%BUILD_NUMBER%
if %errorlevel% neq 0 (
    echo Build failed. 
) else (
    docker push mindmatrix/forward-auth:%BUILD_NUMBER%
    kubectl set image deployment/forward-auth forward-auth=mindmatrix/forward-auth:%BUILD_NUMBER%
