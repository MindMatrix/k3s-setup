...
REM docker build . -t mindmatrix/proxy:%BUILD_NUMBER%
dotnet publish src\proxy.csproj --os linux --arch x64 -c Release -p:ContainerImageTag=%BUILD_NUMBER%
if %errorlevel% neq 0 (
    echo Build failed. 
) else (
    docker push mindmatrix/proxy:%BUILD_NUMBER%
